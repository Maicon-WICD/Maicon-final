...
parser.add_argument("--logdir", type=str, default="/workspace/data/PReNet/logs/PreNet_test_cut_pnsr/", help='path to model and log files')
parser.add_argument("--data_path", type=str, default="/workspace/data/Sample_Submission/", help='path to training data')
parser.add_argument("--save_path", type=str, default="./results/result", help='path to save results')
...
def main():
    ...
    if opt.use_GPU:
        model = model.cuda()
    model.load_state_dict(torch.load(os.path.join(opt.logdir, 'net_iter35.pth')))
    model.eval()
